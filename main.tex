...
Now we can try to discretize the equation \ref{eqn:generalized eigenvalue problem} and solve it numerically. The only remaning issue is that both $H$ and $D^2$ still share a kernel where $z$ is a constant, which makes this eigenvalue problem singular. We bypass this issue very easily in our case because we are discretizing the problem using spherical harmonics and 

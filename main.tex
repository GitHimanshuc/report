...
% Note that the small-magnitude eigenvalues are clumped together while the large eigenvalues are well separated.
...
We see that finding the three smallest-magnitude eigenvalues is much slower than finding the three largest-magnitude eigenvalues (19.6ms vs 1930ms). But if we use the shift-invert transform, which essentially transforms the smallest-magnitude eigenvalues into the largest-magnitude ones, ARPACK becomes much more efficient. This is just an artificial example to illustrate the importance of the shift-invert transform and the speedups that can be achieved. In our case, for a black hole with spin $(0,0,0.5)$, we get a speedup of around 5 times for $ L=15$ and around 10 times for $L=55$, with the speedup continuing to increase as the matrix size grows.

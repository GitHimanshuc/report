...
\Me{Although it should be possible to parallelize the
...
There is no unambiguous way to define the spin of a dynamical black hole horizon in a BBH system. We usually compute a quasilocal spin definition that reduces to the expected well-defined value in the limit of infinite separation. AKV spin ($\chi_{AKV}$) is one of these quasilocal spin definitions, which is constructed by using approximate Killing vectors of the black hole horizons.
...
D_{(A} \phi_{B)}=0,
\end{align}
where $D$ is the spatial covariant derivative on the two-dimensional horizon surface.
...
From Eq.~(\ref{eqn:shear norm in terms of H and z}) and Eq.~(\ref{eqn:generalized eigenvalue problem}) we can see that the amount of shear is proportional to the magnitude of $\lambda$. Therefore, once we have discretized Eq.~(\ref{eqn:generalized eigenvalue problem}), the next step is to find the three eigenvalues with the smallest magnitude and their corresponding eigenfunctions. We can use these eigenfunctions $z$ to compute the spin about the corresponding axis. However, we still need to determine the spin magnitude by normalizing the eigenfunctions, which is equivalent to fixing the value of $N$ in Eq.~(\ref{eqn:Optimizer with the lagrange multiplier and N}). To fix this normalization, we use
...
From the previous Section~\ref{sec:Current AKV algorithm}, we saw that the primary cause of the poor scaling of the current AKV algorithm is the LAPACK routine \texttt{dggev}, which solves for the whole eigensystem. To find the AKV spin we only require the three smallest eigenvalues and their corresponding eigenvectors. There are standard iterative algorithms, implemented in libraries like ARPACK, that can find just the required eigenpairs very efficiently. For a highly deformed horizon, where $L\approx100$, \texttt{dggev} finds all the $\approx 10^4$ eigenpairs and we can reduce the required computation by a lot if we only find the three required eigenpairs using ARPACK.
...
A typical iteration of ARPACK solving a shift-invert system is shown in Fig.(\ref{fig:ARPACK_call_graph_shift_invert_generalized}). Note that it requires us to solve a linear system $(M-\sigma B)y=B x$. For $\sigma=0.1$, ARPACK usually needs around 30 iterations to get the three smallest eigenvalues and their corresponding eigenvectors. Of course, we do not want to take the inverse of the matrix explicitly, so two options are solving the linear system using LU decomposition or using iterative methods. For reasons mentioned in Appendix \ref{sec:Matrix-free version of the AKV algorithm}, we will stay away from the iterative methods and focus on using LU decomposition for solving the linear system.
...
$L\sim O(1000)$, well beyond our expected use cases.
...
The primary conclusion remains the same, though. The ARPACK + LU factorization path is much faster than the LAPACK (\texttt{dggev}) path, and the main bottleneck now is the matrix generation. What does change is the value of $L$ where LU decomposition becomes the bottleneck. This goes from $O(1000)$ to $\approx 200$. For all practical purposes, $L \approx 200$ is still too big. For all current HPC systems, we will be in the regime shown in Fig.(\ref{fig:HPC_all_in_one}), where the matrix generation remains the bottleneck for much larger values of $L$.
...
ARPACK mode & Runtime \\
\midrule
Largest three eigenvalues & 19.6 ms \\
Smallest three eigenvalues & 1.93 s \\
...
We see that finding the three smallest eigenvalues is much slower than finding the three largest eigenvalues (19.6ms vs 1930ms). But if we use the shift-invert transform, which essentially transforms the smallest eigenvalues into the largest ones, ARPACK becomes much more efficient. This is just an artificial example to illustrate the importance of the shift-invert transform and the speedups that one can achieve. In our case, for a black hole with spin $(0,0,0.5)$, we get a speedup of around 5 times for $L = 15$ and around 10 times for $L=55$, with the speedup continuing to increase with the matrix size.

...
\section{\label{sec:Speed difference between finding the smallest and the largest eigenvalues} Speed difference between finding the smallest-magnitude and the largest eigenvalues}

ARPACK converges much faster when we solve for the largest eigenvalues, and they are well separated. This can be seen in the code below, where we construct an artificial matrix and use ARPACK to find the largest and the smallest-magnitude eigenvalues.
...
We see that finding the three smallest-magnitude eigenvalues is much slower than finding the three largest eigenvalues (19.6ms vs 1930ms). But if we use the shift-invert transform, which essentially transforms the smallest-magnitude eigenvalues into the largest ones, ARPACK becomes much more efficient. This is just an artificial example to illustrate the importance of the shift-invert transform and the speedups that one can achieve. In our case, for a black hole with spin $(0,0,0.5)$, we get a speedup of around 5 times for $L = 15$ and around 10 times for $L=55$, with the speedup continuing to increase with the matrix size.
